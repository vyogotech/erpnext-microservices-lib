...
def get_user_tenant_id(user_email=None):
    ...
    try:
        ...
        if result and result[0].get('tenant_id'):
            ...
            # Reject SYSTEM tenant (security check)
            if tenant_id == 'SYSTEM':
                logger.error(
                    f"CRITICAL: User '{user_email}' has SYSTEM tenant_id - rejecting to prevent data leakage")
                return None

            # Allow Administrator if they have a valid tenant_id (not SYSTEM)
            # This allows Administrator to work with tenant-aware services when properly configured
            logger.info(
                f"Successfully resolved tenant_id: {tenant_id} for user: {user_email}")
            return tenant_id

        # For Administrator, provide a helpful error message
...
class TenantAwareDB:
    ...
    def set_value(self, doctype, name, fieldname, value=None, **kwargs):
        """
        Set value for a document with tenant validation
        """
...
class MicroserviceApp:
    ...
    def _isolate_microservice_apps(self):
        ...
        # Filter hooks BEFORE they're resolved to prevent AppNotInstalledError
        # This is more efficient than catching errors during resolution
        original_get_doc_hooks = frappe.get_doc_hooks
        
        def microservice_get_doc_hooks():
            """
            Filter doc_events hooks to only include hooks from installed apps.
            This prevents AppNotInstalledError when hooks reference apps like saas_platform
            that are only installed in Central Site, not in microservices.
            """
            # Get all hooks
            all_hooks = original_get_doc_hooks()
            
            # Filter hooks to only include those from installed apps
            filtered_hooks = {}
            for doctype, events in all_hooks.items():
                ...
                for event, handlers in events.items():
                    ...
                    for handler in handlers:
                        # Extract app name from handler path
                        if '.' in handler:
                            app_name = handler.split('.')[0]
                            # Only include hooks from installed apps
                            if app_name in microservice_apps or app_name == 'frappe':
                                filtered_handlers.append(handler)
                            else:
                                self.logger.debug(
                                    f"Filtering out hook '{handler}' - app '{app_name}' not in allowed apps: {microservice_apps}")
                        else:
                            # If we can't determine the app, include it (might be a built-in)
                            filtered_handlers.append(handler)
        ...
        # Also override frappe.get_attr as a safety net (in case hooks are accessed directly)
        original_get_attr = frappe.get_attr
        
        def microservice_get_attr(method_string):
            """
            Wrapper around frappe.get_attr that handles hooks from non-installed apps.
            This is a safety net in case hooks are accessed directly (not through get_doc_hooks).
            """
            # Extract app name from path
            if '.' in method_string:
                app_name = method_string.split('.')[0]
                # Check if this app is in our allowed list
                if app_name not in microservice_apps and app_name != 'frappe':
                    # This hook references a non-installed app - raise AttributeError
                    # Frappe will catch this and skip the hook gracefully
                    self.logger.debug(
                        f"Skipping hook '{method_string}' - app '{app_name}' not in allowed apps: {microservice_apps}")
                    raise AttributeError(f"Hook from non-installed app '{app_name}' skipped")
            
            # For allowed apps, try to get the attr
            try:
                return original_get_attr(method_string)
            except frappe.AppNotInstalledError as e:
                # If it's an AppNotInstalledError, raise AttributeError so Frappe skips it
                app_name = method_string.split('.')[0] if '.' in method_string else 'unknown'
                self.logger.debug(
                    f"Skipping hook '{method_string}' - app '{app_name}' not installed: {e}")
                raise AttributeError(f"Hook from non-installed app '{app_name}' skipped") from e
            except Exception:
                # For other errors, re-raise (don't silently fail)
                raise
        ...
        self.logger.info(
            f"frappe.get_attr wrapped to skip hooks from non-installed apps")

    def _validate_oauth_token(self, access_token: str):
        """
        Validate OAuth2 access token against Central Site
        
        Args:
            access_token: The Bearer token from the Authorization header
            
        Returns:
            tuple: (username, error_response)
        """
        try:
            # Call Central Site OIDC profile endpoint
            ...
            if response.status_code == 200:
                ...
                # OAuth profile usually returns email or sub as username
    ...
    def _validate_session(self):
        """
        Validate user session via OAuth2 Bearer token OR SID cookie
        """
        try:
            # 1. Check for OAuth2 Bearer token first (API clients/Kong)
            auth_header = request.headers.get('Authorization', '')
            if auth_header.startswith('Bearer '):
                access_token = auth_header[7:]  # Remove 'Bearer ' prefix
                return self._validate_oauth_token(access_token)

            # 2. Fall back to SID cookie (browser sessions)
    ...
    def register_resource(self, doctype, base_path=None, methods=None, custom_handlers=None):
        ...
        if 'GET' in methods:
            ...
            # Add Swagger documentation for list
            ...
            # Add Swagger documentation for get
        ...
        if 'POST' in methods:
            ...
            # Add Swagger documentation for create
        ...
        if 'PUT' in methods:
            ...
            # Add Swagger documentation for update
        ...
        if 'DELETE' in methods:
            ...
            # Add Swagger documentation for delete
    ...
    def run(self, **kwargs):
        ...
        # Create site_config.json if it doesn't exist (automation)
        try:
            create_site_config()
        ...
        # Prepare run arguments, prioritizing kwargs
        run_args = {
